...
const previews = [
  { num: '01', to: '/about',    title: 'About',    desc: 'Background, experience timeline, NAID internship, teaching, and team leadership.' },
  { num: '02', to: '/skills',   title: 'Skills',   desc: '.NET, Node.js, Angular, SQL Server, Redis, Docker — 29+ technologies.' },
  { num: '03', to: '/projects', title: 'Projects', desc: 'Microservices, Clean Architecture APIs, production portals, and university work.' },
  { num: '04', to: '/contact',  title: 'Contact',  desc: 'Open to roles, freelance, or collaboration. Let\'s build together.' },
]
  ...
  return (
    <main ref={ref} className="overflow-x-hidden">
      ...
      <section className="relative w-full h-screen flex items-center overflow-hidden">
        ...
        <div className="relative z-10 w-full md:w-[58%] px-6 md:px-10 lg:px-16 max-w-7xl flex flex-col justify-center">
          ...
          <div className="h-tag flex items-center gap-3 mb-10">
            ...
            <span className="ml-2 flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-[#7c6aff] animate-pulse" />
          ...
          <p className="h-quote text-[clamp(0.85rem,1.6vw,1.05rem)] text-[#6b7280] italic mb-6 max-w-sm leading-relaxed">
          ...
          <div className="h-meta flex flex-wrap items-center gap-3 mb-10">
            {['.NET Backend Engineer', 'CS Student', 'Instructor', 'Team Lead'].map((t, i) => (
              <span key={t}>
                ...
                {i < 3 && <span className="text-[#1f2434] ml-3">·</span>}
              </span>
            ))}
          </div>

          <div className="flex flex-wrap gap-3 mb-8">
            <Link to="/projects" className="h-cta px-7 py-3 bg-[#7c6aff] text-white text-[11px] tracking-[0.2em] uppercase font-bold hover:bg-[#6c5ce7] transition-colors duration-300 rounded">
              View Work
            </Link>
            <Link to="/about" className="h-cta px-7 py-3 border border-[#1f2434] text-[#6b7280] text-[11px] tracking-[0.2em] uppercase hover:border-[#7c6aff] hover:text-[#a78bfa] transition-all duration-300 rounded">
              About Me
            </Link>
            <a
          ...
          <div className="h-cta flex gap-6">
            {[
              ...
              <a key={label} href={href}
                ...
                rel="noreferrer"
                className="text-[10px] tracking-[0.3em] uppercase text-[#374151] hover:text-[#a78bfa] transition-colors duration-300 border-b border-[#1f2434] pb-0.5 hover:border-[#7c6aff]"
              >{label}</a>
            ))}
          </div>
        </div>
      ...
      {/* ── MARQUEE ── */}
      <div className="border-y border-[#1f2434] py-4 overflow-hidden">
        <div className="marquee-track">
          {[
            ...
            <span key={i} className="text-[11px] tracking-[0.25em] uppercase text-[#1f2434] shrink-0">
      ...
      <section className="prev-section max-w-7xl mx-auto px-6 md:px-10 lg:px-16 py-28">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {previews.map((p) => (
            <Link key={p.num} to={p.to} className="prev-card card p-6 flex flex-col gap-5 rounded-lg group hover:border-[#7c6aff]/40 transition-all duration-300">
              <div className="flex items-start justify-between">
                <span className="text-4xl font-black text-[#1f2434] group-hover:text-[#2a3045] transition-colors">{p.num}</span>
              ...
              <div className="mt-auto pt-4 border-t border-[#1f2434] group-hover:border-[#7c6aff]/30 transition-colors">
                <span className="text-[10px] tracking-[0.3em] uppercase text-[#2a3045] group-hover:text-[#7c6aff] transition-colors">Explore →</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      {/* ── STATS ── */}
      <section className="stats-section border-t border-[#1f2434]">
